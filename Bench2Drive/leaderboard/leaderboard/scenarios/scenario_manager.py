...
class ScenarioManager(object):
    ...
    def _tick_scenario(self):
        ...
        if self._timestamp_last_run < timestamp.elapsed_seconds and self._running:
            ...
            if isinstance(ego_action, tuple) and len(ego_action) == 2 and ego_action[0] == "OPEN_LOOP":
                open_loop_transform = ego_action[1]
                if open_loop_transform is not None:
                    self.ego_vehicles[0].set_transform(open_loop_transform)
            else:
